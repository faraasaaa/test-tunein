...
export const downloadSong = async (track: Track): Promise<DownloadedSong | null> => {
  try {
    ...
    const downloadApiUrl = `https://necessary-arlene-noneoool-8661e22a.koyeb.app/get?id=${trackId}`;
    const response = await fetch(downloadApiUrl);

    if (!response.ok) {
      const errorData = await response.json().catch(() => null);
      if (errorData && errorData.error === "Track with the specified Spotify ID not found.") {
        throw new Error("Song not in DB");
      }
    ...
    if (error instanceof Error && error.message === 'Song not in DB') {
        Alert.alert('Song Not Found', `This song isn't in our database yet. Please contact the owner to have it added.`);
