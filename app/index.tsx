...
export default function SearchScreen() {
  ...
  const renderTrackItem = ({ item }: { item: Track }) => {
    ...
    const downloadedVersion = downloadedSongs.find((ds) => {
      const titleMatch =
        ds.title.toLowerCase().trim() === item.name.toLowerCase().trim();
      const albumMatch =
        ds.album.toLowerCase().trim() === item.album.toLowerCase().trim();
      const artistMatch =
        ds.artist.toLowerCase().trim() === artistsText.toLowerCase().trim();
